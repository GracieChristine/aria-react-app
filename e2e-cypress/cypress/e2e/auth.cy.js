describe('Auth', () => {
  ...
  describe('Register', () => {
    ...
    it('should error out if required full name missing', () => {
      ...
      cy.get('input[name="email"]').type(`new-user-${ts}@example.com`)
  ...
  describe('Login', () => {
    ...
    before(() => {
      cy.request('POST', '/api/auth/register', {
        ...
        email: `register-user-${ts}@example.com`, password: 'Password123!'
      })
    })
    ...
    it('should error out if required password missing', () => {
      cy.get('input[name="email"]').type(`register-user-${ts}@example.com`)
      cy.get('input[name="password"]').clear();
      cy.get('button[type="submit"]').click()
    ...
    it('should error out if user isn\'t register yet', () => {
      ...
      cy.get('input[name="email"]').type(`unregister-user-${ts}@example.com`)
      cy.get('input[name="password"]').type('Password123!')
      cy.get('button[type="submit"]').click()
  ...
  describe('Logout', () => {
    it('should logout as a login user and redirect to home @smoke', () => {
      ...
      cy.get('input[name="email"]').type(`test-user-1-${ts}@example.com`)
      cy.get('input[name="password"]').type('Password123!')
      cy.get('input[name="confirmPassword"]').type('Password123!')
      cy.get('button[type="submit"]').click()
    ...
    it('should logout as a new registered user and redirect to home', () => {
      ...
      cy.get('input[name="email"]').type(`test-user-2-${ts}@example.com`)
      cy.get('input[name="password"]').type('Password123!')
      cy.get('input[name="confirmPassword"]').type('Password123!')
      cy.get('button[type="submit"]').click()
