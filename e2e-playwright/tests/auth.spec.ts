...
test.describe('Auth', () => {
  ...
  test.describe('Register', () => {
    ...
    test('should error out if required full name missing', async ({ page }) => {
      ...
      await page.locator('input[name="email"]').fill(`new-user-${ts}@aria.com`);
  ...
  test.describe('Login', () => {
    test.beforeAll(async ({ request }) => {
      await request.post('/api/auth/register', {
        data: {
          firstName: 'Register',
          lastName: 'User',
          email: `register-user-${ts}@aria.com`,
    ...
    test('should error out if required password missing', async ({ page }) => {
      await page.locator('input[name="email"]').fill(`register-user-${ts}@aria.com`);
      await page.locator('input[name="password"]').clear();
      await page.locator('button[type="submit"]').click();
    ...
    test("should error out if user isn't registered yet", async ({ page }) => {
      await page.locator('input[name="email"]').fill(`unregister-user-${ts}@aria.com`);
      await page.locator('input[name="password"]').fill('Password123!');
      await page.locator('button[type="submit"]').click();
  ...
  test.describe('Logout', () => {
    test('should logout as a login user and redirect to home @smoke', async ({ page }) => {
      ...
      await page.locator('input[name="email"]').fill(`test-user-1-${ts}@aria.com`);
      await page.locator('input[name="password"]').fill('Password123!');
      await page.locator('input[name="confirmPassword"]').fill('Password123!');
      await page.locator('button[type="submit"]').click();
    ...
    test('should logout as a new registered user and redirect to home', async ({ page }) => {
      ...
      await page.locator('input[name="email"]').fill(`test-user-2-${ts}@aria.com`);
      await page.locator('input[name="password"]').fill('Password123!');
      await page.locator('input[name="confirmPassword"]').fill('Password123!');
      await page.locator('button[type="submit"]').click();
